...
{
	ExceptableResult result = NewER();
	Bytes list = ( Bytes ) malloc ( sizeof ( struct BS ) );
	if ( list == NULL )
		{
			result->Exception = NewException ( GL_MALLOC_FAILED_ID, GL_MALLOC_FAILED_DESC );
			result->Content = NULL;
	...
	list->HEAD = ( byte* ) malloc ( GL_SIZE* sizeof ( byte ) );
	if ( list->HEAD == NULL )
		{
			free ( list );
			result->Exception = NewException ( GL_HEAD_MALLOC_FAILED_ID, GL_HEAD_MALLOC_FAILED_DESC );
...
ExceptableResult AddByte ( Bytes List, byte b )
{
	...
	if ( List->Count >= List->Size )
		{
			byte* temp= ( byte* ) realloc ( List->HEAD, ( List->Size+GL_SIZE ) *sizeof ( byte ) );
			if ( temp == NULL )
				{
					ExceptableResult result = NewER();

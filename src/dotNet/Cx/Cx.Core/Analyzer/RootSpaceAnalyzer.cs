...
namespace Cx.Core.Analyzer
{
    public class RootSpaceAnalyzer : CAnalyzer
    {
        ...
        public override OperationResult<(bool, Symbol?)> Analyze(AnalyzerProvider provider , int Pos , ref TreeNode node)
        {
            ...
            for (int i = 0 ; i < node.Children.Count ; i++)
            {
                ...
                {
                    var AR=analyzer.Analyze(provider,Position,ref item);
